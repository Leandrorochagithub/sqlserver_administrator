...
DECLARE @BackupType NVARCHAR(50) = 'Diff';
DECLARE @BackupPath NVARCHAR(256) = 'C:\Program Files\Microsoft SQL Server\MSSQL16.SQLEXCHANGERATE\MSSQL\Backup\crypto_db\Database\';
DECLARE @DateTimeString NVARCHAR(50) = FORMAT(GETDATE(), 'yyyy-MM-dd-HHmmss');
DECLARE @BackupFileName NVARCHAR(256)= @DatabaseName + '_' + @BackupType + '_' + @DateTimeString + '.bak';
...
TO DISK = @BackupPath
WITH DIFFERENTIAL, COMPRESSION, -- Tipo e compressão para reduzir tamanho do arquivo
